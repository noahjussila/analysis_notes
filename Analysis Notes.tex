...
\begin{definition}
	Suppose $ f $ is a bounded real function  on the interval $ [a,b] $. Define the values $$\lowint_a^b f(x)\ dx=\sup\limits_{P\in\textbf{P}([a,b])} L(P,f), \ \ \ \ \upint_a^b f(x)\ dx =\inf\limits_{P\in\textbf{P}([a,b])} U(P,f). $$ 
...
\end{enumerate}
\end{proof}
This proposition seems like a random string of inequalities,\footnote{To be honest, most of real analysis seems like a random string of inequalities sometimes.} but...
...
$$ \int_{a}^{b}f(x)\ dx=\int_{\phi(a)}^{\phi(b)}f(\phi(y))\phi'(y)\ dy$$
...
\begin{example}[Unbounded Function]
	Let $ f(x)=1/x $. This function cannot be integrated on $ [0,1] $, as it is unbounded on this interval, because $ f(x-)=\infty $.  
\end{example}
